...
// ---------------------------------------------------------
// computeNeighborsAndRecs: función auxiliar para benchmarking
// ---------------------------------------------------------

func computeNeighborsAndRecs(
	...
	targetData, ok := userRatings[targetUser]
	if !ok {
		return nil, fmt.Errorf("usuario %s no encontrado", targetUser)
	}

	// Canal para distribuir trabajo y recibir resultados
	type job struct {
		user string
		data map[string]map[string]float64
	}
	jobs := make(chan string, workers)
	results := make(chan network.NeighborResult, len(userRatings))

	// Crear pool de workers
	for w := 0; w < workers; w++ {
		go func() {
			for other := range jobs {
				if other == targetUser {
					continue
				}
				sim := CosineSimilarity(targetData, userRatings[other])
	...
	// Calcular cuantos resultados esperamos (N - 1)
	expectedResults := len(userRatings) - 1

	// Recoger EXACTAMENTE (N - 1) resultados
	allNeighbors := make([]network.NeighborResult, 0, expectedResults)
	for i := 0; i < expectedResults; i++ {
		n := <-results
		allNeighbors = append(allNeighbors, n)
	}

	// Seleccionar top-K vecinos
	topK := TopK(allNeighbors, k)

	// Predecir ratings
	recs := PredictRatings(targetUser, userRatings, topK)

	// Tomar N recomendaciones finales (puedes ajustar a 10)
	finalRecs := TopNRecommendations(recs, 10)

	return finalRecs, nil
}

// RunKNNBenchmark hace llamadas a la lógica KNN existente para benchmarking
func RunKNNBenchmark(userRatings map[string]map[string]float64, users []string, workers int, k int) {
	for _, u := range users {
		_, _ = computeNeighborsAndRecs(userRatings, u, workers, k)
	}
}
